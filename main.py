...
from algorithm.cup_normalize import CUP_normalize
...
def train(args):
    ...
    if args.algo == "CUP":
        agent = CUP(env, policy, value_net, cvalue_net,
                    ...
                    args.pi_grad_norm, args.project_grad_norm,
                    args.vf_grad_norm, args.cvf_grad_norm,
    ...
    elif args.algo == "CUP_normalize":
        agent = CUP_normalize(env, policy, value_net, cvalue_net,
                                    pi_optimizer, vf_optimizer, cvf_optimizer,
                                    args.pi_grad_norm, args.project_grad_norm,
                                    args.vf_grad_norm, args.cvf_grad_norm,
                                    args.num_epochs, args.mb_size,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta', type=float, default=0.02,
                        help='KL bound used in Policy Update for early stop (default: 0.02)')
    ...
    parser.add_argument('--batch-size', type=int, default=5000,
                        help='Batch Size per Update (default: 5000)')
    ...
    parser.add_argument('--algo', type=str, default="CUP_clip",
                        help='algo choosing from [CUP, CUP_clip, CUP_normalize]')
    parser.add_argument('--kl-coef', type=float, default=0.3,
                        help='kl_coef in Policy Improvement')
    parser.add_argument('--clip', type=float, default=0.2,
                        help='clip coefficient')
    parser.add_argument('--pi-grad-norm', type=float, default=0.5,
                        help='Actor max gradient norm in policy improvement')
    parser.add_argument('--project-grad-norm', type=float, default=0.5,
                        help='Actor gradient norm in projection')
    parser.add_argument('--vf-grad-norm', type=float, default=0.5,
                        help='Reward Critic gradient norm')
    parser.add_argument('--cvf-grad-norm', type=float, default=0.5,
                        help='Cost critic gradient norm')
